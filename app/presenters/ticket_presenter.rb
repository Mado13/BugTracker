class TicketPresenter
  ...
  def lead_developer_select_tag
    if @user.admin? || @user.project_manager?
       h.select_tag 'project[lead_developer_id]',
      ...
      h.hidden_field_tag 'project[lead_developer_id]', @user.id
    end
  end
  ...
  def hidden_tag_status
    if @ticket.new_record?
      h.hidden_field_tag 'project[status]', 'Open'
    else
      h.select_tag 'project[status]',
